...
reactive_stimuli <- function(stimuli_function, stimuli_reactive, prepared_stimuli, body = NULL) {
  ...
  print('daboom')
  print(stimuli_reactive)
  print(prepared_stimuli)
  if (str_contains(stimuli_reactive, "reactive")) {
    print('daooo2')
    print(prepared_stimuli)
...
retrieve_page_type <- function(page_type_string, stimuli_wrapped, special_page_underlying_page_type = "one_button_page",
                               page_text = "Click to hear the stimuli", page_title = " ", interactive, stimuli, stimuli_reactive = FALSE, answer_meta_data = NULL, ...) {
  ...
  else if(stimuli_reactive == FALSE & page_type_string == "record_audio_page" | stimuli_reactive == FALSE & page_type_string == "record_midi_page") {
...
present_stimuli_static <- function(stimuli, stimuli_type, display_modality, page_type, ...) {
  ...
  # musical stimuli types
    ...
    return_stimuli <- present_stimuli_midi_notes(stimuli, display_modality, get_answer, page_type = page_type, ...)
  }
...
present_stimuli <- function(stimuli, stimuli_type, display_modality, page_type = "one_button_page",
                            ...
                            answer_meta_data = NULL, get_answer = NULL, stimuli_reactive = FALSE, ...) {

  # reactive stimuli i.e that requires something at run time, in a reactive_page
  if (stimuli_reactive == FALSE) {
    return_stimuli <- present_stimuli_static(stimuli, stimuli_type, display_modality, page_type, ...)
  }
  else {
    return_stimuli <- present_stimuli_reactive(stimuli_reactive, stimuli, stimuli_type, display_modality, page_type, ...)
  }
  ...
  else if(page_type == "record_midi_page" | page_type == "record_audio_page") {
