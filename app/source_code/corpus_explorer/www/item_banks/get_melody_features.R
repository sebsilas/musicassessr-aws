...
get_stimuli_length <- function(melody_col, sep) {
  ...
  res <- lapply(melody_col, function(x) length(unlist(strsplit(x, sep))))
  res <- as.numeric(res)
  res
}
...
get_tonality <- function(melody, sep) {
  ...
  if (!is.na(melody)) {
    pitch <- str.mel.to.vector(melody, sep)
    len <- length(pitch)
...
get_step_contour <- function(melody) {
...
#input is a vector of string, of the form [i_1, i_2,...], where the i's are integer semitoneintervals.
...
get_melody_features <- function(df, mel_sep = "-", durationMeasures = FALSE) {
  ...
  df <- bind_cols(df, tonality)

  # step contour
  step_contour_df <- bind_rows(lapply(df$melody, function(x) get_step_contour(str.mel.to.vector(x, mel_sep))))
  ...
  df <- df %>% mutate_at(vars(log_freq, step.cont.glob.dir, step.cont.glob.var,
                              step.cont.loc.var, tonal.clarity, tonal.spike, tonalness), funs(round(., 2)))
