...
var user_input = []; // SJS: create an empty array to record user incoming pitches
var user_input_midi = []; // incoming pitches transformed to MIDI
var pitches_times = []; // SJS: create an empty array to record the timestamp associated with those pitches
var trial_no = 8; // set to a value for testing
var max_notes = 6; // max notes to load per melody. also used for number of notes to stream
...
var test_freqs = [349.23, 369.99, 391.99];

var test_convert = test_freqs.map(freq_to_midi);

console.log(test_convert);


// load text file of stimuli for calidation

function loadJSON(callback) {

    var xobj = new XMLHttpRequest();
    xobj.overrideMimeType("application/json");
    xobj.open('GET', './stimuli/Berkowitz_stim.json', true); // location of stimuli in text form
...
var stimuli_list = null;
var target_melody = null;
...
function run(response) {
    ...
    target_melody = stimuli_list[trial_no].slice(0, max_notes);
    console.log(target_mel);
    if(target_melody.includes(user_input_midi) === false) {
        updateErrorCount()
    }
    
    user_input_midi = [20, 40, 50];
    
    compareMel = valMelody(target_melody, user_input_midi);
    
    console.log(compareMel);
    crepe();
...
// load a particular melody from stimuli list
...
function valMelody(target_melody, user_input_midi) {
    for (var i = 0; i < target_melody.length; i++)
        if (target_melody[i] != user_input_midi[i])
            return "False";
    return "True";
}
    ...
    function process_microphone_buffer(event) {
        resample(event.inputBuffer, function (resampled) {
            tf.tidy(() => {
                ...
                // SJS: user input
                var result_processed = (confidence > 0.5) ? predicted_hz.toFixed(3) : 'x'; // SJS: the result that will actually be used for data analysis. append x if low conf
                var user_input_stream = user_input.slice(Math.max(user_input.length - max_notes, 0)); // SJS: stream of just the last n values

                if (result_processed != 'x') {
                    user_input.push(result_processed); // SJS: add incoming pitches to array   
                    var midi_processed = pitch.fromFreq(result_processed);
                    user_input_midi.push(midi_processed);
                    document.getElementById('user_input_display').innerHTML = user_input_midi.join(" "); // SJS: update display
                    //console.log(user_input) // SJS: print to console
                    pitches_times.push(performance.now()); // SJS: add times associated with incoming pitches to array
                    //console.log(performance.now()) // SJS: report experiment elapsed time?
                    document.getElementById('time_array').innerHTML = pitches_times.join(" "); // SJS: update display
                    document.getElementById('stream').innerHTML = user_input_stream.join(" "); // SJS: update display
                }
